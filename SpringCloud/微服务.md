...
SpringCloud-Stream 屏蔽各中间件厂商的API调用区别，提供统一的API接口

`learn-spring-cloud-stream-producer`
...
`learn-spring-cloud-stream-consumer`
...
Nacos集注册中心和配置中心于一体，提供了管理后台

Sentinel用于服务治理，提供管理后台来动态配置服务熔断与配置

服务调用支持REST，支持Netflix的feign,支持Dubbo的无缝融合

### 注册中心Nacos

Nacos做注册中心，可直接下载使用，不需要单独作为一个微服务开发
...
`learn-alibaba-discovery-provide`
...
spring:
  ...
  cloud:
    nacos:
      ...
      config:
        server-addr: 127.0.0.1:8848
        # nacos 中默认 Data ID=${spring.application.name}.properties，Group=DEFAULT_GROUP
        # nacos 默认的配置文件后缀为 .properties
        file-extension: yml
```
...
```xml
      ...
      <!-- API接口 -->
       <dependency>
            <groupId>learn.springcloud.alibaba</groupId>
            <artifactId>learn-alibaba-dubbo-api</artifactId>
...
dubbo:
  scan:
    # 配置dubbo要发布的API实现类扫描
    base-packages: learn.alibaba.dubbo.consumer.service
    # name dubbo 支持 dubbo http rmi hession 等协议
    # -1   通信端口递增
  protocol:
    name: dubbo
    port: -1
  registry:
     #注册中心地址，使用SpringCloud注册中心
    address: spring-cloud://localhost
  cloud:
    #订阅的微服务名称，作为生产者可为NULL
    subscribed-services:
```
...
dubbo:
  scan:
    base-packages: learn.alibaba.dubbo.provide.service
  protocol:
    name: dubbo
    port: -1
  registry:
  ...
  cloud:
    #订阅的微服务名称
    subscribed-services: nacos-discovery-provide
```

### 服务治理

SpringCloud-Alibaba 提供了治理模块sentinel，并提供了管理后台，可直接下载使用
...
如上配置，需要注意的是只有对应的微服务接收了首次请求才会在sentinel面板中出现

对于controller层可直接在面板中配置，而若想要对service层进行监控限流则需要@SentinelResource注解，才能被sentinel发现
